<template>
  <div :class="dark ? 'dark' : 'light'">
    <div class="fixed w-full h-full top-0 left-0 bg-zn-400 flex flex-col items-center justify-center dark:bg-zn-800 transition duration-500">
      <NuxtLink to="/" class="fixed top-2 left-2 font-mont font-semibold text-zn-800 dark:text-zn-450 text-sm hover:underline hover:text-zn-900 dark:hover:text-zn-300 cursor-pointer transition duration-200">{{ $t('go_back') }}</NuxtLink>
